const resolvers = ({
    ...
    Mutation: {
        addCidadao:  async(_, {itm},    { dataSources }) => {
            await dataSources.app.addCidadao(itm);
        },
        toggleCidadao: async(_, {IdCidadao},    { dataSources })    =>  {
            await dataSources.app.toggleCidadao({IdCidadao})
        }
    },
